...
var forRadar = data => {
    return {
        labels: ['POINTS', 'REBOUNDS', 'ASSISTS', 'BLOCKS', 'STEALS', 'TURNOVERS', 'FOULS'],
        datasets: [
            {
                label: data.team_name.split(' ')[1],
                backgroundColor: 'rgba(255, 99, 132, 0.2)',
                data: [data.pts, data.reb, data.ast, data.blk, data.stl, data.tu, data.pf]
            },
            {
                label: 'Opponent',
                backgroundColor: 'rgba(140, 37, 244, 0.2)',
                data: [data.vs_pts, data.vs_reb, data.vs_ast, data.vs_blk, data.vs_stl, data.vs_tu, data.vs_pf]
            }
        ],
    };
};
...
var getTeamName = teams => {
    var result = [];

    teams.forEach(team => {
        result.push(team.first_name + ' ' + team.last_name);
    });
